...
function NUIService.NUISetNearPlayers(obj, nearestPlayers)
    ...
    if not NPlayerSelector then
        print("^1[ERROR]^0 NPlayerSelector is not initialized. Check the mega_nplayerselector setup.")
        return
    end
    ...
    -- Usa NPlayerSelector per selezionare un giocatore
    ...
    if isvalid then
        for _, player in ipairs(nearestPlayers) do
            if GetPlayerServerId(player) == tonumber(data.player) then
                ...
                -- Giocatore esegue l'animazione
                local playerPed = PlayerPedId()
                local animDict = "mech_butcher"
                local animName = "small_tin_give_player"
                RequestAnimDict(animDict)
                while not HasAnimDictLoaded(animDict) do
                    Wait(10)
                end

                -- Esegui l'animazione senza movimento
                ...
                -- Riapri l'inventario dopo l'animazione
                NUIService.OpenInv()
